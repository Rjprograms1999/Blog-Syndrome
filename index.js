...
// declaring blogposts array where every post will be stored on runtime
const blogPosts = [];

// featPosts array is predefined
const featPosts = {
  posts: [
    {
...
// to set the public folder in express to serve static files
...
app.get("/editPost", (req, res) => {
  ...
  if (post) {
    res.render("editPost.ejs", { post: post, postIndex: postIndex }); // Pass the post data and index to the edit page
  }
});
...
app.post("/publish", upload.single("image"), (req, res) => {
  //it will take form data from the user i.e their blog post info) once the publish button is clicked
...
app.post("/delete-post", (req, res) => {
  ...
  blogPosts.splice(postIndex, 1); // Remove no of items starting from postIndex item
